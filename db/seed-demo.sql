-- db/seed-demo.sql
--
-- DEMO seed — extends core seed with example fields and operations.
-- Intended for development, staging, and product demonstrations.
-- Must NOT be run on a production instance with real agronomist data.
--
-- Idempotent: fields and operations blocks are skipped if those tables
-- already contain rows.  Run core seed first (it is called by seed-demo.js).


-- ── Demo fields ───────────────────────────────────────────────────────────────
-- Variety IDs are resolved by name so this works across environments
-- regardless of serial sequence state.
...
-- ── Demo operations ───────────────────────────────────────────────────────────
-- Only inserted when the operations table is empty.
-- Field IDs are resolved by field name.
